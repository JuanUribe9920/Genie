...
.proc-banner {
    ...
    align-items: center;
    gap: 1.2rem;
    background: linear-gradient(135deg, #111113, #0e0e10);
...
.proc-banner .proc-lamp svg {{
    filter: drop-shadow(0 0 10px {GOLD}88);
}}
.proc-banner .proc-title {{
    font-size: 0.95rem;
    font-weight: 600;
    color: {PEARL};
    margin-bottom: 0.2rem;
}}
.proc-banner .proc-msg {{
    font-size: 0.78rem;
    color: {MUTED};
...
.proc-banner .proc-title {{ animation: pulse-gold 1.8s ease-in-out infinite; }}
...
LAMP_MINI = f"""<svg width="38" height="38" viewBox="0 0 52 52" fill="none" xmlns="http://www.w3.org/2000/svg">
  <defs><radialGradient id="gm" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="{GOLD}" stop-opacity="0.25"/><stop offset="100%" stop-color="{GOLD}" stop-opacity="0"/></radialGradient></defs>
  <circle cx="26" cy="24" r="22" fill="url(#gm)"/>
  <path d="M26 6C17 6 12 15 12 22C12 31 19 39 26 43C33 39 40 31 40 22C40 15 35 6 26 6Z" stroke="{GOLD}" stroke-width="1.2" fill="none" stroke-linejoin="round"/>
  <path d="M36 38L44 44" stroke="{GOLD}" stroke-width="1.2" stroke-linecap="round"/>
  <path d="M12 25C7 25 5 29 5 32C5 35 7 38 12 38" stroke="{GOLD}" stroke-width="1.2" stroke-linecap="round" fill="none"/>
  <circle class="lamp-flame" cx="26" cy="22" r="2.5" fill="{GOLD}" opacity="0.75"/>
</svg>"""

def proc_banner(step: str) -> str:
    return (f'<div class="proc-banner">'
            f'<div class="proc-lamp">{LAMP_MINI}</div>'
            f'<div><div class="proc-title">Genie está procesando</div>'
            f'<div class="proc-msg">{step}</div></div></div>')
...
def build_vectorstore(uploaded_files: list, api_key: str = "") -> tuple:
    ...
    for uf in uploaded_files:
        banner.markdown(proc_banner(f"Leyendo <b>{uf.name}</b>…"), unsafe_allow_html=True)
        ...
        try:
            ...
            state_ph.markdown(state_done(f"{uf.name} leído"), unsafe_allow_html=True)
    ...
    # ── Split ────────────────────────────────────────────────────────────────
    banner.markdown(proc_banner("Dividiendo contenido en fragmentos…"), unsafe_allow_html=True)
    state_ph2 = st.empty()
    ...
    # ── Embed ────────────────────────────────────────────────────────────────
    banner.markdown(proc_banner("Creando base de conocimiento…"), unsafe_allow_html=True)
    state_ph3 = st.empty()
    ...
    state_ph3.markdown(state_done("Base de conocimiento lista"), unsafe_allow_html=True)
    banner.markdown(proc_banner("¡Listo! Genie está listo para responder ✦"), unsafe_allow_html=True)
...
def ask_genie(vectorstore, question: str, api_key: str) -> tuple[str, list[str]]:
    ...
    prompt = f"""Eres Genie, un asistente experto en analizar documentos. Responde en español de forma clara y precisa basándote ÚNICAMENTE en el contexto proporcionado. Si la respuesta no está en el contexto, dilo claramente.
